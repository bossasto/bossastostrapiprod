...
module.exports = {
  ...
  async me(ctx) {
    ...
    if (userResponse.role && userResponse.role.name === 'Worker' && userResponse.worker) {
      const worker = await strapi.query('worker').findOne({ id: userResponse.worker });
      userResponse.worker = worker;
      delete userResponse.company;
      delete userResponse.worker.user;
    } else if (userResponse.role && userResponse.role.name === 'Company' && userResponse.company) {
      const company = await strapi.query('company').findOne({ id: userResponse.company });
      userResponse.company = company;
      delete userResponse.worker;
      userResponse.company.users = userResponse.company.users.map(companyUser => sanitizeUser(companyUser));
    }
